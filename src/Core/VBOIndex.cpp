...
bool getSimilarVertexIndex(glm::vec3& in_vertex, glm::vec2& in_uv, glm::vec3& in_normal,
                           ...
                           std::vector<glm::vec3>& out_normals, index_t& result) {
  // Lame linear search
  for (unsigned int i = 0; i < out_vertices.size(); i++) {
    if (is_near(in_vertex.x, out_vertices[i].x) && is_near(in_vertex.y, out_vertices[i].y) &&
        is_near(in_vertex.z, out_vertices[i].z) && is_near(in_uv.x, out_uvs[i].x) && is_near(in_uv.y, out_uvs[i].y) &&
        is_near(in_normal.x, out_normals[i].x) && is_near(in_normal.y, out_normals[i].y) &&
...
void indexVBO_TBN(std::vector<glm::vec3>& in_vertices, std::vector<glm::vec2>& in_uvs,
  ...
  for (unsigned int i = 0; i < in_vertices.size(); i++) {
    ...
    if (found) {  // A similar vertex is already in the VBO, use it instead !
      ...
      out_indices.push_back((index_t)out_vertices.size() - 1);
    }
  }
}
